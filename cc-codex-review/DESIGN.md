...
> 进度:**P0 ✅(v0.4.0)、P2 ✅(v0.5.0/v0.5.1/v0.6.0);P1 🟡 instrumentation 已实现(v0.7.0)、真机数据采集待做;P3 🟡 脚手架已实现(v0.8.0)、实验数据待采;P4 未做(暂缓)**。优先级与语义经对抗式互审锁定。
...
- **P3 首轮遗漏检查实验 — 🟡 脚手架已实现(v0.8.0),实验数据待采**:第 1 轮追加**一次**"针对当前证据与目标的遗漏检查"(**不**预判投机二阶问题、**不**输出 completeness 自评分)。A/B:同任务等额轮数预算配对运行、**纳入 UNRESOLVED 样本**、未收敛率作门禁指标、相同预算快照统一比较(converged/有效issue/噪音/Σwall-clock);"有效 issue" 须经**盲评或固定 rubric 终局复核**(仅"被采纳"不算),并记反向指标"不必要修订"。质量优先决策规则。
  ...
  - **⏳ 待办**:用真 `/cc-codex-review:review --omission-check` 跑 ≥3 个真实任务的配对 A/B(纳入 UNRESOLVED 样本),由盲评/rubric 标注 `effective` + 反向指标 `unnecessary_revisions`,喂 `experiment.mjs decide` 出质量优先结论。
