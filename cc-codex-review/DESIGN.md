...
- **② Hook 硬强制收敛诚实性(v0.14.0,经 brainstorm + do 协作 + 自审 dogfood)**:①(review-audit)能独立重放,但仍在 Claude 流程内、可跳过(不审直接文本写 RESOLVED 即逃逸)。② 用 **Claude Code Stop hook** 把它升级为**回合级硬门禁**。spec:`docs/specs/2026-06-29-hook-hard-enforcement-design.md`。
  ...
  - **fail-open 于基础设施 + 有界 IO**:hook 自身跑不起来(读不到 transcript / import 失败 / stdin 坏)→ 放行——避免全局 hook 把用户**整个 Claude Code** 卡死;**仅"能跑出审计结论且未通过"才 block**。**IO 有界(整体复核 I1 修)**:transcript **尾读**(末尾 512KB,不随会话线性增长)、manifest/raw 读前 `lstat` 须普通文件 + 大小上限(防 FIFO/特殊文件阻塞、超大文件 OOM)。
  - **诚实边界(关键,不夸大)**:**显著提高门槛、非 bulletproof**。残余:① **部分修(v0.14.2)**:写了规范 §7 成功结论头「✅…收敛结论(状态:RESOLVED)」却无末行哨兵 → 现 hook **block**(堵"忘出哨兵/偷懒不审"的最可能漏法);但**任意散文**声称"已解决/通过"仍无法安全识别(广匹配会误拦设计讨论)→ 固有;② 指向旧的通过 manifest 重放(临时文件易失缓解、未绑本次材料指纹);③ manifest 漏末尾轮(①受信边界残余);④ 基础设施 fail-open 可被故意弄坏 hook 环境绕过;⑤ 伪造通过需连 raw Codex 落盘一起骗(难)。**净效果:从"随手假 RESOLVED"变成"必须主动造假(撤哨兵/伪造 hash 匹配产出/弄坏 hook 环境)"**,常见/偷懒路径被回合级硬门挡住。彻底闭合需外部 runner / 真盲审,门槛更高、本期不做。
  - **②② 残余 #2/#3 的修法已对齐(transcript 证据绑定,v0.14.2 仅记机制、未实现)**:经 do 协作,Claude×Codex 一致认为 #2(旧 manifest 重放)/#3(漏报已实际跑过的末尾轮)在**纯插件层可实质修**——`codex-round` 成功输出加 `audit_event{run_id,round_index,out_sha256,thread_id,...}`;review 每轮带同一 `run_id`、manifest 记之;hook 从**本回合 transcript 的真实 tool-result**(非 assistant 文本)提取这些 audit_event,要求与 manifest **按 run_id/轮序/sha 精确一致**(transcript 有更晚轮而 manifest 漏列 → block)。**因其耦合 Claude Code transcript tool-result schema(跨版本可能漂移→全局 hook 误拦)+ blast radius 高,列为单独谨慎一轮、未在本期实现**;残余仍含"本回合内伪造/无关材料收敛"(无外部 runner 不可证)。
  ...
  - 新增 `scripts/enforce-resolved-hook.mjs` + `hooks/hooks.json` + `tests/enforce-resolved-hook.test.mjs` 16 例(无哨兵放行 / 哨兵+通过放行 / raw rejected block / manifest 缺失/无 kind/sha 不符 block / 代码块内/散文中间哨兵不误拦 / 旧哨兵不误拦后续回合 / transcript·stdin fail-open / **有界 IO:transcript 非普通文件 fail-open、manifest 非普通 block、超大 transcript 尾读仍检出** / **截断绕过:单条超大记录带哨兵→block、无哨兵→放行**);review.md §6/§7 接入哨兵+manifest 落盘;全套 **247 绿**。
  - **v0.14.1(整体 review 两大版本时自审,Codex 3 轮收敛 RESOLVED——且经本插件已激活的 Stop hook 自我把关)**:抓出并修 hook 的两个真问题:I1(major:全局 Stop hook 无界/阻塞式 IO〔读整 transcript、信任意 manifest/raw 路径、无 regular-file/上限〕→ 改 transcript 尾读 + manifest/raw `lstat` 普通文件 + 大小上限)、I2(major:尾读+JSONL 解析下,单条超大最终记录被截断→解析失败→静默当无哨兵放行的绕过 → 改为末行不可解析且原始尾部含哨兵标记时保守 block)。
