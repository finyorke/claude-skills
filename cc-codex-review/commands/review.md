...
- **收敛诚实性独立审计(①「运动员兼裁判」缓解,`${CLAUDE_PLUGIN_ROOT}/scripts/review-audit.mjs`)**:`converge` 虽确定性,但它吃的是**你转述的**每轮 Codex 字段;为让"收敛是否诚实"可独立核验——
  ...
  - 维护一份 **audit manifest**:`{kind:"cc-codex-review-audit-manifest", claudeAgree:<bool>, rounds:[{round_index:<1-based 连续>, codex_out:"<rN.json 路径>", codex_out_sha256:"<该轮 64 hex 哈希,必填>", claude_actions:{adopted,rebutted,merges,annotations}}]}`——**manifest 只放你自己的动作,绝不放你转述的 Codex verdict/disposition/issues**(那些审计器只从 raw 文件读)。`kind` 字段必填(② Stop hook 据此确认指向的是本工具的审计 manifest)。
  ...
  - **②硬强制:manifest 落盘 + 哨兵**(配合插件自带 Stop hook,见 DESIGN §12 ②):审计通过后,把这份 manifest **写到临时文件**(如 `/tmp/cc-manifest-<时间戳>.json`);§7 宣布 RESOLVED 时输出哨兵行 `<<CCR-RESOLVED manifest="<该 manifest 绝对路径>">>`。Stop hook 会在回合结束时**独立重跑 `review-audit`** 校验——审计没真过却写哨兵会被**硬 block 不让结束回合**。故:只有真过审计才写哨兵;UNRESOLVED **绝不**写哨兵。
  - **诚实边界**:本审计是**重放引擎**;它本身仍在你流程内、可被跳过。**回合级硬门禁由 ② 插件自带 Stop hook 提供**(见下「②硬强制」/ DESIGN §12 ②):hook 在回合结束独立重跑本审计,假 RESOLVED 会被硬 block。即便如此仍非 bulletproof(不出哨兵+散文写 RESOLVED 可逃逸等,见 DESIGN §12 ② 残余),但已把假收敛从"随手"变为"须主动造假"。
...
- 收敛成功:打印
  ...
  **并在结论末尾输出哨兵行**(②硬强制,见 §6):`<<CCR-RESOLVED manifest="<§6 落盘的 manifest 绝对路径>">>`——插件 Stop hook 会据此独立重审,假 RESOLVED 会被硬 block。
