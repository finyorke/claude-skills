...
- 确认后调 `node "${CLAUDE_PLUGIN_ROOT}/scripts/decisions-log.mjs" upsert`,stdin `{"repo":"<生效repo>","ops":[...]}`:新决策/未决用 `{op:"append",entry:{source:"do",statement:"<一句话>",status:"decided",rationale:"…",severity:"major"}}`(open 则 `status:"open",positions:{claude:"…",codex:"…"},severity`);**entry 必填 `source`/`statement`/`status`(decided 还需 `rationale`),`id`/`ts` 脚本自动分配勿传,严重度用 `severity` 字段勿塞 rationale**;某 `open` 本轮谈拢→`{op:"set-status",id:旧open_id,status:"decided",rationale:"..."}` **原地翻**(带 rationale,不堆条);某旧决策被**不同新决策**推翻→append 新 entry 带 `supersedes:[旧id]`(旧条渲染时自动隐藏);某决策**不再适用**(功能删除/并入更大规则、无替换)→`{op:"set-status",id,status:"closed",rationale:"退役理由"}` 退役隐藏(**已实现但仍生效的约束保持 decided 别 closed**)。脚本会写 jsonl + 重渲染 `decisions.md`。
