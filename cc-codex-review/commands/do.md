...
- 确认后调 `node "${CLAUDE_PLUGIN_ROOT}/scripts/decisions-log.mjs" upsert`,stdin `{"repo":"<生效repo>","ops":[...]}`:新决策/未决用 `{op:"append",entry:{source:"do",statement:"<一句话>",status:"decided",rationale:"…",severity:"major"}}`(open 则 `status:"open",positions:{claude:"…",codex:"…"},severity`);**entry 必填 `source`/`statement`/`status`(decided 还需 `rationale`),`id`/`ts` 脚本自动分配勿传,严重度用 `severity` 字段勿塞 rationale**;某 `open` 本轮谈拢→`{op:"set-status",id:旧open_id,status:"decided",rationale:"..."}` **原地翻**(带 rationale,不堆条);某旧决策被**不同新决策**推翻→append 新 entry 带 `supersedes:[旧id]`(旧条渲染时自动隐藏)。脚本会写 jsonl + 重渲染 `decisions.md`。
