...
- 整套工具**被它自己审过**(dogfood)。
- **防假互审**:`review`/`do` 的结论会附 codex `thread_id`,可在 `~/.codex/sessions` 核对"真的调了 Codex"(`verify-codex-session.mjs` + 收敛门禁:没真调就不许判 RESOLVED)。注:这让假互审**可被核对、成本更高,但非杜绝**——塞一个历史真 thread_id、或干脆不调核对,仍可绕,根治需 hook。
